# Microll

[![Build Status](https://travis-ci.com/IceTimux/microll.svg?branch=master)](https://travis-ci.com/IceTimux/microll)

Microll aggregates various RSS feeds into a single feed for consumption.

## Demo
https://microll.herokuapp.com/

## Get started

### Requirements
Please make sure your operating system has the following software installed:
* [Bundler](https://bundler.io/)

### Install
Run `bundle install` in the root directory of the repo to install the dependencies.

### Usage
Run `rackup config.ru` in the root direcetory to start the web application.

### Deployment
This repository comes with a rackup file named `config.ru` in the root directory used for deploymemt.
